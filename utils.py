...
def load_tickers():
    ...
    try:
        ...
        pass
    try:
        return list(SET100_TICKERS)
    except Exception:
        return []

def get_fear_and_greed_index():
    """
    Fetches the Fear & Greed Index from alternative source or API.
    Since CNN's API is protected, we can use an alternative free API or scrape.
    Returns: {
        'score': float,
        'rating': str,
        'timestamp': str
    ...
    """
    try:
        # Using alternative API: fear-and-greed-index.p.rapidapi.com or similar
        # For simplicity without API key, we can fetch from alternative open source
        # Here we use a public api endpoint from alternative provider
...
SET100_META_FILE = "set100_meta.json"

def _now_bangkok_iso():
    import datetime
    import pytz
    return datetime.datetime.now(pytz.timezone("Asia/Bangkok")).isoformat()

def load_set100_meta():
    import json
    import os
    if not os.path.exists(SET100_META_FILE):
        return {}
    try:
        with open(SET100_META_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            return data if isinstance(data, dict) else {}
    except Exception:
        return {}

def save_set100_meta(meta: dict):
    import json
    try:
        with open(SET100_META_FILE, "w", encoding="utf-8") as f:
            json.dump(meta, f, ensure_ascii=False)
    except Exception:
        pass

def _set100_extract_download_url(html_text: str):
    import re
    import html as htmlmod
    m = re.search(
        r'href="(/services/download\?url=https://media\.set\.or\.th/set/Documents/[^"]+?\.pdf&amp;name=[^"]+)"',
        html_text,
    )
    if not m:
        return None
    return "https://www.set.or.th" + htmlmod.unescape(m.group(1))

def fetch_latest_set100_symbols_from_set():
    import hashlib
    import re
    ...
    list_urls = [
        "https://www.set.or.th/th/market/information/securities-list/constituents-list-set50-set100",
    ...
    if not html_text:
        raise RuntimeError("ดึงหน้า SET สำหรับรายชื่อ SET50/SET100 ไม่สำเร็จ")

    download_url = _set100_extract_download_url(html_text)
    if not download_url:
        raise RuntimeError("หาลิงก์ PDF รายชื่อ SET50/SET100 ล่าสุดไม่เจอ (โครงสร้างหน้าอาจเปลี่ยน)")

    pdf_res = s.get(download_url, timeout=60)
    if pdf_res.status_code != 200 or not pdf_res.content:
        raise RuntimeError("ดาวน์โหลด PDF รายชื่อ SET50/SET100 ไม่สำเร็จ")

    pdf_bytes = pdf_res.content
    pdf_sha256 = hashlib.sha256(pdf_bytes).hexdigest()

    qs = parse_qs(urlparse(download_url).query)
    doc_name = (qs.get("name") or [None])[0]
    raw_pdf_url = (qs.get("url") or [None])[0]

    try:
        from pypdf import PdfReader
    except Exception as e:
        raise RuntimeError("ต้องติดตั้งแพ็กเกจ pypdf เพื่ออ่านไฟล์ PDF") from e
    ...
    for i, page in enumerate(reader.pages):
        ...
        t_u = t.upper()
        if "SET100" in t_u and ("SET100FF" in t_u or "SET100 / SET100FF" in t_u or "SET100/SET100FF" in t_u):
    ...
    if not target_pages:
        raise RuntimeError("หา section SET100 ใน PDF ไม่เจอ (รูปแบบเอกสารอาจเปลี่ยน)")

    num_to_sym = {}
    for i in target_pages:
        t = reader.pages[i].extract_text() or ""
        for n, sym in re.findall(r"\b(\d{1,3})\s+([A-Z0-9-]{1,15})\b", t.upper()):
            n_i = int(n)
            if 1 <= n_i <= 100 and sym not in {"SET", "SET50", "SET100", "SET100FF"}:
                num_to_sym.setdefault(n_i, sym)

    symbols = [num_to_sym.get(i) for i in range(1, 101)]
    if any(s is None for s in symbols):
        missing = [i for i, s in enumerate(symbols, start=1) if s is None]
        raise RuntimeError(f"ดึงรายชื่อ SET100 ได้ไม่ครบ 100 ตัว (ขาดลำดับ: {missing})")

    symbols = [s.replace(".BK", "").strip() for s in symbols]
    symbols = [s for s in symbols if s]
    if len(set(symbols)) != 100 or len(symbols) != 100:
        raise RuntimeError("รายชื่อ SET100 ที่ดึงได้มีจำนวนไม่ครบหรือซ้ำ (PDF parse ผิดพลาด)")
...
def update_set100_tickers_if_needed(force: bool = False, max_age_hours: int = 24):
    import datetime

    now_iso = _now_bangkok_iso()
    meta = load_set100_meta()

    if not force:
        last_checked = meta.get("last_checked_at")
        if isinstance(last_checked, str) and last_checked:
            try:
                dt = datetime.datetime.fromisoformat(last_checked)
                age_hours = (datetime.datetime.fromisoformat(now_iso) - dt).total_seconds() / 3600
    ...
    current_norm = [t.replace(".BK", "").upper().strip() for t in current if isinstance(t, str)]
    current_norm = [t for t in current_norm if t]

    try:
        latest, info = fetch_latest_set100_symbols_from_set()
    except Exception as e:
        meta_new = {
        ...
        save_set100_meta(meta_new)
        return {"ok": False, "changed": False, "error": str(e)}

    latest_norm = [t.replace(".BK", "").upper().strip() for t in latest if isinstance(t, str)]
    latest_norm = [t for t in latest_norm if t]

    added = sorted(set(latest_norm) - set(current_norm))
    removed = sorted(set(current_norm) - set(latest_norm))
    changed = bool(added or removed) or (len(current_norm) != 100) or (len(latest_norm) != 100)

    if changed:
        save_tickers(latest_norm)

    meta_new = {
        "ok": True,
        "last_checked_at": now_iso,
        "last_updated_at": now_iso if changed else meta.get("last_updated_at"),
        "changed": changed,
        "added": added,
        "removed": removed,
        **info,
    ...
    return {
        ...
        "last_checked_at": now_iso,
        "last_updated_at": meta_new.get("last_updated_at"),
    }
    ...
    if txn_type == 'Sell':
        ...
        for t in portfolio:
            if t.get('symbol', '').upper() != symbol_u:
                continue
            t_type = t.get('transaction_type', 'Buy')
            if t_type == 'Buy':
                held_qty += int(t.get('qty', 0))
            elif t_type == 'Sell':
                held_qty -= int(t.get('qty', 0))
        if int(qty) > held_qty:
            return False, f"จำนวนขายมากกว่าที่ถืออยู่ (ถือ {held_qty:,} ขาย {int(qty):,})"
    record = {
        ...
        "price": float(price),
        "qty": int(qty),
        "transaction_type": txn_type # Avoid keyword 'type'
    }
    ...
    for t in transactions:
        ...
        if txn_type == 'Buy':
            ...
            if holdings[sym]['qty'] > 0:
                ...
                sell_qty = min(t['qty'], holdings[sym]['qty'])
                holdings[sym]['qty'] -= sell_qty
                holdings[sym]['total_cost'] -= avg_cost * sell_qty
                if holdings[sym]['total_cost'] < 0:
                    holdings[sym]['total_cost'] = 0
...
def get_eps_10_years(ticker_list, years=10):
    """
    Fetches historical EPS for multiple tickers using thaifin.
    ...
    for ticker in ticker_list:
        try:
            # Strip .BK for thaifin
            t_main = ticker.replace('.BK', '')
            stock = thaifin.Stock(t_main)
            df = stock.yearly_dataframe
            
            history_data = {}
            if df is not None and not df.empty and 'earning_per_share' in df.columns:
                # Convert PeriodIndex to list of years (int) for easier lookup
                df_years = {p.year: float(val) for p, val in zip(df.index, df['earning_per_share']) if not pd.isna(val)}
                for year in target_years:
                    if year in df_years:
                        history_data[year] = df_years[year]
                    else:
                        history_data[year] = 0.0
            else:
                for year in target_years:
                    history_data[year] = 0.0
                    
            # --- Fetch Financial Ratios (Stats) ---
            # Using yfinance for stats as thaifin might not have all current stats easily accessible
            stats = {}
            try:
                t_yf = ticker
                if not t_yf.endswith('.BK'): t_yf += '.BK'
                yf_stock = yf.Ticker(t_yf)
                info = yf_stock.info
                stats = {
            ...
            print(f"Error fetching thaifin data for {ticker}: {e}")
            # Fallback to empty structure
            history_data = {y: 0.0 for y in target_years}
            results[ticker] = {'history': history_data, 'stats': {}}

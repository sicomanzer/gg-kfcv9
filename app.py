...
def ensure_set100_universe():
    return utils.update_set100_tickers_if_needed(force=False)

SET100_UPDATE = ensure_set100_universe()
...
MARKET_DATA_TTL_SECONDS = 1800

@st.cache_data(ttl=MARKET_DATA_TTL_SECONDS)
def fetch_raw_market_data(tickers):
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        ...
        future_to_ticker = {executor.submit(utils.get_stock_data, ticker): ticker for ticker in tickers}
...
def fetch_set_index_metrics():
    try:
        ...
        rsi14 = float(rsi14_s.iloc[-1]) if not rsi14_s.empty and pd.notna(rsi14_s.iloc[-1]) else None
        dist200 = (current / sma200 - 1) * 100 if sma200 else None
...
# Load Pipeline
raw_data_list, last_fetch_time = fetch_raw_market_data(tuple(SET100_TICKERS))
...
if not df.empty:
    ...
    df['margin_of_safety_raw'] = df['margin_of_safety']
    df['margin_of_safety_safe'] = df['margin_of_safety'].fillna(-100)
    ...
    df['score_mos'] = df['margin_of_safety_safe'].apply(lambda x: 1 if x > 0 else 0)
...
if page == "📊 แดชบอร์ดภาพรวม":
    ...
    with c_market_1:
        ...
        if vix_data:
            ...
            if vix_price < 20:
                ...
                vix_status = "ตลาดเริ่มกลัว"
                vix_color = "#ef4444" # Red
                vix_bg = "rgba(239, 68, 68, 0.1)"
                vix_icon = "😨"
                vix_desc = "VIX สูงกว่า 30: ความกลัวปกคลุมตลาด มีโอกาสเทขาย"
            else: # > 40
                vix_status = "วิกฤต (Panic)"
                vix_color = "#b91c1c" # Dark Red
                vix_bg = "rgba(185, 28, 28, 0.1)"
                vix_icon = "🔥"
                vix_desc = "VIX สูงกว่า 40: ตลาดแตกตื่นรุนแรง"
            ...
            with st.container():
                ...
                <div style="background-color: {vix_bg}; padding: 15px; border-radius: 10px; border: 1px solid {vix_color}; margin-bottom: 20px;">
                    <h3 style="margin: 0; color: {vix_color};">⚡ ดัชนีความผันผวน (VIX Index)</h3>
                </div>
        ...
        if fng_data:
            ...
            with st.container():
                ...
                <div style="background-color: {fng_bg}; padding: 15px; border-radius: 10px; border: 1px solid {fng_color}; margin-bottom: 20px;">
                    <h3 style="margin: 0; color: {fng_color};">🌡️ ดัชนีความกลัวและความโลภ (Fear & Greed)</h3>
                </div>
                """, unsafe_allow_html=True)
    ...
    if vix_data and fng_data:
        ...
        sentiment_status = "ปกติ"
        sentiment_color = "#6b7280" # Gray
        sentiment_action = "รอดูสถานการณ์ / ลงทุนตามแผนปกติ"
        sentiment_desc = "ตลาดอยู่ในภาวะปกติ ไม่มีความกลัวหรือความโลภที่รุนแรงมากนัก เน้นคัดเลือกหุ้นรายตัว"
        
        # 1. Super Buy (Panic + Extreme Fear) -> Buy when others are fearful
        if v_price >= 30 and f_score <= 25:
            sentiment_status = "🔥 โอกาสทอง"
            sentiment_color = "#047857" # Dark Green
            sentiment_action = "กล้าซื้อสวนตลาด"
            sentiment_desc = "ทุกคนกลัวสุดขีด + ตลาดเทขายรุนแรง = **เวลาที่ดีที่สุดในการซื้อหุ้นดีราคาถูก** (Warren Buffett: 'จงโลภเมื่อผู้อื่นกลัว')"
            
        # 2. Buy/Accumulate (Moderate Fear)
        elif (v_price >= 20) and (f_score < 45):
            sentiment_status = "✅ น่าสะสม"
            sentiment_color = "#10b981" # Green
            sentiment_action = "ซื้อเมื่อย่อตัว"
        ...
        # 3. Warning/Sell (Complacency + Extreme Greed) -> Sell when others are greedy
            ...
            sentiment_status = "⚠️ ระวังแรงขาย"
            sentiment_color = "#ef4444" # Red
            sentiment_action = "ขายทำกำไร / ชะลอการซื้อ"
            sentiment_desc = "ตลาดนิ่งนอนใจ + มีความโลภสูง = **ความเสี่ยงในการปรับฐานสูง** (Warren Buffett: 'จงกลัวเมื่อผู้อื่นโลภ')"
            
        # 4. Danger Zone (Extreme Complacency / Bubble)
        elif v_price < 15 and f_score >= 85:
             sentiment_status = "⛔ ฟองสบู่/ความเสี่ยงสูง"
             sentiment_color = "#b91c1c" # Dark Red
             sentiment_action = "ถือเงินสด / ระวังติดดอย"
             sentiment_desc = "ตลาดมั่นใจเกินเหตุ + โลภสุดขีด = **ระวังการปรับฐานรุนแรง**"

        # 5. Mixed Signals (High Volatility but High Greed? Rare)
        elif v_price > 30 and f_score > 60:
            sentiment_status = "😵 ตลาดสับสน/ผันผวน"
            sentiment_color = "#f59e0b" # Orange
            sentiment_action = "จับตาดูใกล้ชิด"
    ...
    st.markdown("---")
    st.subheader("🇹🇭 ตัวชี้วัดตลาดหุ้นไทย (SET Index)")
    set_metrics = fetch_set_index_metrics()
    if set_metrics:
        set_price = set_metrics["current"]
        set_rsi = set_metrics["rsi14"]
        set_dist200 = set_metrics["dist200"]
        set_sma50 = set_metrics["sma50"]
        set_sma200 = set_metrics["sma200"]
        hist = set_metrics["history"]

        signal = "ถือ"
        signal_color = "#6b7280"
        signal_desc = "สัญญาณเป็นกลาง เน้นทยอยลงทุนตามแผน"

        if set_dist200 is not None and set_rsi is not None:
            if set_dist200 <= -8 and set_rsi <= 35:
                signal = "ทยอยซื้อสะสม"
                signal_color = "#10b981"
                signal_desc = "ดัชนีต่ำกว่าเส้นค่าเฉลี่ยระยะยาวมาก และ RSI ต่ำ: เหมาะกับทยอยสะสมแบบมีวินัย"
            elif set_dist200 <= -3 and set_rsi <= 45:
                signal = "ซื้อ"
                signal_color = "#34d399"
                signal_desc = "ดัชนีต่ำกว่าค่าเฉลี่ยระยะยาวและเริ่มอ่อนแรง: ซื้อแบบคุมความเสี่ยง"
            elif set_dist200 >= 10 and set_rsi >= 65:
                signal = "ขาย"
                signal_color = "#ef4444"
                signal_desc = "ดัชนีสูงกว่าค่าเฉลี่ยระยะยาวมาก และ RSI สูง: ระวังแรงขาย/ขายทำกำไรบางส่วน"

        c_set1, c_set2, c_set3, c_set4 = st.columns(4)
        c_set1.metric("SET Index", f"{set_price:,.2f}")
        c_set2.metric("สัญญาณ", signal)
        if set_dist200 is not None:
            c_set3.metric("ห่าง SMA200", f"{set_dist200:+.2f}%")
        else:
            c_set3.metric("ห่าง SMA200", "N/A")
        if set_rsi is not None:
            c_set4.metric("RSI (14)", f"{set_rsi:.1f}")
        else:
            c_set4.metric("RSI (14)", "N/A")

        st.markdown(f"""
        <div style="padding: 12px; border-radius: 10px; border: 2px solid {signal_color}; background-color: rgba(255,255,255,0.05);">
            <div style="font-size: 18px; font-weight: bold; color: {signal_color};">สรุป: {signal}</div>
            <div style="margin-top: 6px;">{signal_desc}</div>
            <div style="margin-top: 8px; color: gray; font-size: 12px;">
                เกณฑ์: ระยะห่างจาก SMA200 + RSI(14) (เป็นสัญญาณเชิงเทคนิคเพื่อช่วยตัดสินใจ ไม่ใช่คำแนะนำการลงทุน)
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if hist is not None and not hist.empty and "Close" in hist.columns:
            ...
            fig_set = go.Figure()
            fig_set.add_trace(go.Scatter(x=chart_df.index, y=chart_df["Close"], mode="lines", name="SET", line=dict(color="#2563eb", width=2)))
            if set_sma50 is not None:
                fig_set.add_trace(go.Scatter(x=chart_df.index, y=chart_df["SMA50"], mode="lines", name="SMA50", line=dict(color="#f59e0b", width=1)))
            if set_sma200 is not None:
                fig_set.add_trace(go.Scatter(x=chart_df.index, y=chart_df["SMA200"], mode="lines", name="SMA200", line=dict(color="#6b7280", width=1)))
            fig_set.update_layout(height=260, margin=dict(l=10, r=10, t=30, b=10), legend=dict(orientation="h"))
            st.plotly_chart(fig_set, use_container_width=True)
    else:
        st.warning("⚠️ ไม่สามารถดึงข้อมูล SET Index ได้ในขณะนี้")
    
    if not df.empty:
        # --- NEW: SUPER DASHBOARD KPIs ---
        st.markdown("### 🌐 ภาพรวมตลาด")
        
        # Calculate market stats safely
        total_mcap = df['marketCap'].sum() / 1e9 # in Billions THB
        df['P/E_raw'] = df.apply(lambda row: row['price']/row['trailingEps'] if pd.notna(row.get('trailingEps')) and row['trailingEps'] > 0 else None, axis=1)
        ...
        kpi1.metric("มูลค่าตลาดรวม (SET100)", f"฿{total_mcap:,.0f} B", help="รวมมูลค่า Market Cap ของหุ้นทั้งหมดในลิสต์")
        kpi2.metric("P/E เฉลี่ย (Median)", f"{median_pe:.2f} เท่า", help="ค่ามัธยฐานของ P/E ตลาด")
        kpi3.metric("ปันผลเฉลี่ย (Yield)", f"{avg_div:.2f}%", help="อัตราเงินปันผลตอบแทนเฉลี่ย")
        kpi4.metric("MOS เฉลี่ยตลาด", f"{avg_mos_all:.2f}%", delta=f"{avg_mos_all:.2f}%", delta_color="normal", help="Margin of Safety เฉลี่ย (ยิ่งบวกเยอะยิ่งถูก)")
        
        # --- NEW: MARKET TREEMAP & SCATTER ---
        st.markdown("### 🗺️ แผนที่ค้นหาหุ้น")
        ...
        with c_map1:
            ...
            fig_tree = px.treemap(
                ...
                hover_data={'mos_clipped': False, 'price': True, 'VI Score': True, 'margin_of_safety': ':.2f'},
                title="ขนาดกล่อง = มูลค่าตลาด | สีเขียว = ราคาถูก (MOS สูง)"
            )
            fig_tree.update_layout(margin=dict(t=30, l=0, r=0, b=0), height=450)
        ...
        with c_map2:
            ...
            # Add quadrant lines
            fig_scatter.add_vline(x=0, line_width=1, line_dash="dash", line_color="red")
            fig_scatter.add_hline(y=6, line_width=1, line_dash="dash", line_color="red")
            
            # Annotations for quadrants
            fig_scatter.add_annotation(x=25, y=9.5, text="✨ หุ้นเทพ (ถูกและดี)", showarrow=False, font=dict(color="#10b981", size=14, weight="bold"))
            fig_scatter.add_annotation(x=-25, y=9.5, text="🏆 หุ้นดีแต่แพง", showarrow=False, font=dict(color="#f59e0b", size=14))
            fig_scatter.add_annotation(x=25, y=1, text="⚠️ Value Trap (ถูกแต่แย่)", showarrow=False, font=dict(color="#f59e0b", size=14))
            fig_scatter.add_annotation(x=-25, y=1, text="🗑️ หลีกเลี่ยง (แพงและแย่)", showarrow=False, font=dict(color="#ef4444", size=14))
            
            # Restrict X axis to avoid extreme outliers zooming out too much
            fig_scatter.update_xaxes(range=[-60, 60])
            fig_scatter.update_yaxes(range=[-0.5, 10.5])
            
            fig_scatter.update_layout(margin=dict(t=30, l=0, r=0, b=0), height=450, legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5))
        ...
        st.markdown(f"### 📢 สรุปโอกาสลงทุนวันนี้")
        st.caption(f"ข้อมูลล่าสุดเมื่อ: {last_fetch_time.strftime('%Y-%m-%d %H:%M:%S')}")

        now_ts = datetime.datetime.now(last_fetch_time.tzinfo) if getattr(last_fetch_time, "tzinfo", None) else datetime.datetime.now()
        cache_age_min = (now_ts - last_fetch_time).total_seconds() / 60 if last_fetch_time else None

        with st.expander("🔎 ตรวจสอบคุณภาพข้อมูล (Data Quality)"):
            st.markdown("**แหล่งข้อมูล**: ราคา/งบ/ตัวชี้วัดปัจจุบันจาก Yahoo Finance (ผ่าน yfinance) + ข้อมูลย้อนหลัง 10 ปีจาก thaifin (ในบางหน้า)")
            if cache_age_min is not None:
                st.markdown(f"**อายุข้อมูลชุดนี้**: {cache_age_min:.0f} นาที")
            st.markdown(f"**Auto refresh (TTL)**: {MARKET_DATA_TTL_SECONDS // 60} นาที (หรือกดปุ่มอัปเดตข้อมูลด้านซ้าย)")

            total_universe = len(SET100_TICKERS)
            fetched_count = len(raw_data_list) if raw_data_list else 0
            st.markdown(f"**จำนวนหุ้นที่ตั้งใจดึง**: {total_universe} | **ดึงได้สำเร็จ**: {fetched_count} | **ขาดหาย**: {max(total_universe - fetched_count, 0)}")
            ...
            issue_rows = []
            for item in (raw_data_list or []):
                if not isinstance(item, dict):
                    continue
                sym = item.get("symbol")
                issue_rows.append({
                    "หุ้น": sym,
                    "Missing Price": is_missing(item.get("price")) or (item.get("price", 0) == 0),
                    "Missing EPS": is_missing(item.get("trailingEps")) or (item.get("trailingEps", 0) <= 0),
                    "Missing BVPS": is_missing(item.get("bookValue")) or (item.get("bookValue", 0) <= 0),
                    "Missing Div": is_missing(item.get("dividendRate")) or (item.get("dividendRate", 0) <= 0),
                    "Missing ROE": is_missing(item.get("returnOnEquity")),
                    "Missing NPM": is_missing(item.get("profitMargins")),
                    "Missing FCF": is_missing(item.get("freeCashflow")),
                    "Missing Price Time": is_missing(item.get("last_price_time")) or (item.get("last_price_time", 0) == 0),
                })

            if issue_rows:
                issue_df = pd.DataFrame(issue_rows)
                summary = {
                    "Price": int(issue_df["Missing Price"].sum()),
                    "EPS": int(issue_df["Missing EPS"].sum()),
                    "BVPS": int(issue_df["Missing BVPS"].sum()),
                    "DividendRate": int(issue_df["Missing Div"].sum()),
                    "ROE": int(issue_df["Missing ROE"].sum()),
                    "NPM": int(issue_df["Missing NPM"].sum()),
                    "FCF": int(issue_df["Missing FCF"].sum()),
                    "LastPriceTime": int(issue_df["Missing Price Time"].sum()),
                }
                st.markdown("**สรุปจำนวนหุ้นที่ข้อมูลขาด (จาก yfinance)**")
                st.dataframe(pd.DataFrame([summary]), hide_index=True, use_container_width=True)

                mos_missing = int(df["margin_of_safety_raw"].isna().sum()) if "margin_of_safety_raw" in df.columns else 0
                st.markdown(f"**หุ้นที่คำนวณ MOS ไม่ได้**: {mos_missing}")

                issue_df["Missing Any"] = issue_df.drop(columns=["หุ้น"]).any(axis=1)
                st.markdown("**รายการที่ควรตรวจสอบ (ข้อมูลขาดอย่างน้อย 1 ช่อง)**")
                st.dataframe(issue_df[issue_df["Missing Any"]].drop(columns=["Missing Any"]).head(30), hide_index=True, use_container_width=True)
        
        with st.expander("ℹ️ อ่านคำแนะนำสัญญาณการลงทุน"):
            st.markdown("""
            **ความหมายของสัญญาณ:**
            *   🟢 **ซื้อสะสม:** หุ้นคุณภาพดี (VI Score ≥ 7) และราคาถูกมาก (MOS > 15%)
            *   🟢 **ซื้อ:** หุ้นคุณภาพดี (VI Score ≥ 7) และราคาถูกกว่ามูลค่าจริง (MOS > 0%)
            *   🟡 **ถือรอ/ชะลอการซื้อ:** หุ้นที่ราคาเริ่มเต็มมูลค่า หรือคุณภาพปานกลาง
            *   🔴 **ขาย/หลีกเลี่ยง:** หุ้นที่ราคาแพงเกินไป (MOS < -20%) หรือพื้นฐานแย่ลง (VI Score < 5)
        ...
        # Filter Lists
        buy_list = df[df['Action'] == 'ซื้อสะสม']
        hold_list = df[df['Action'] == 'ถือรอ']
        sell_list = df[df['Action'] == 'ขาย/หลีกเลี่ยง']
        

        with col_act1:
            st.success(f"🟢 **หุ้นน่าสะสม: {len(buy_list)} ตัว**")
            if not buy_list.empty:
                st.dataframe(
                    buy_list[['symbol', 'price', 'valuation_ddm', 'VI Score']].rename(columns={'symbol':'หุ้น', 'price':'ราคา', 'valuation_ddm':'มูลค่า DDM', 'VI Score':'คะแนน VI'}).style.format({'ราคา': '{:.2f}', 'มูลค่า DDM': '{:.2f}'}), 
                    hide_index=True,
                    height=250
                )
            else:
                st.caption("วันนี้ยังไม่มีหุ้นเข้าเกณฑ์น่าสะสม")
                
        with col_act2:
            st.warning(f"🟡 **หุ้นถือรอ/พักเงิน: {len(hold_list)} ตัว**")
            if not hold_list.empty:
                st.dataframe(
                    hold_list[['symbol', 'price', 'valuation_ddm', 'VI Score']].rename(columns={'symbol':'หุ้น', 'price':'ราคา', 'valuation_ddm':'มูลค่า DDM', 'VI Score':'คะแนน VI'}).style.format({'ราคา': '{:.2f}', 'มูลค่า DDM': '{:.2f}'}), 
                    hide_index=True,
                    height=250
                )
            else:
                st.caption(f"ไม่มีหุ้นที่เข้าเกณฑ์ถือรอ")

        with col_act3:
            st.error(f"🔴 **หุ้นควรระวัง/ขาย: {len(sell_list)} ตัว**")
            if not sell_list.empty:
                st.dataframe(
                    sell_list[['symbol', 'price', 'valuation_ddm', 'VI Score']].rename(columns={'symbol':'หุ้น', 'price':'ราคา', 'valuation_ddm':'มูลค่า DDM', 'VI Score':'คะแนน VI'}).style.format({'ราคา': '{:.2f}', 'มูลค่า DDM': '{:.2f}'}), 
                    hide_index=True,
                    height=250
                )
            else:
        ...
        # Sidebar Filter
        st.sidebar.markdown("---")
        st.sidebar.subheader("🔍 ตัวกรองหุ้น")
        ...
        val_base = st.sidebar.selectbox(
            ...
            index=0, 
            help="ใช้สำหรับคำนวณ MOS% และสีของราคา"
        )
        ...
        st.markdown("---")
        st.subheader("🏆 10 สุดยอดหุ้นแกร่ง")
        st.markdown(f"""
        ...
        if 'sector_th' in df.columns:
            sector_df = df[df['sector_th'] != 'ไม่ระบุ'].groupby('sector_th').agg(
                ...
                avg_vi_score=('VI Score', 'mean'),
                total_mcap=('marketCap', 'sum')
            ).reset_index()
            
            sector_df = sector_df.sort_values(by='avg_mos', ascending=False)
            
            c_sec1, c_sec2 = st.columns([1.5, 1])
            with c_sec1:
                fig_sec = px.bar(
                    sector_df,
                    x='avg_mos',
            ...
            with c_sec2:
                ...
                st.dataframe(
                    display_sec.style.format({
                        ...
                        'P/E เฉลี่ย': '{:.2f}',
                        'VI Score เฉลี่ย': '{:.2f}'
                    }).background_gradient(subset=['MOS เฉลี่ย(%)'], cmap='Greens'),
                    hide_index=True,
                    height=350
                )

        # --- NEW: TOP DIVIDEND & HIGH QUALITY ---
        st.markdown("---")
        st.subheader("💰 หุ้นปันผลเด่น & คุณภาพสูง")
        ...
        with c_div1:
            ...
            if 'dividendYield' in df.columns:
                ...
                if not top_div.empty:
                    ...
                    st.caption("ไม่มีหุ้นที่เข้าเกณฑ์ปันผลสูง")
        
        with c_div2:
            st.markdown("**Top 5 หุ้นคุณภาพสูงสุด (คะแนน VI สูงสุด)**")
            top_qual = df.sort_values(by=['VI Score', 'margin_of_safety'], ascending=[False, False]).head(5)
            st.dataframe(
                top_qual[['symbol', 'VI Score', 'price', 'margin_of_safety', 'returnOnEquity']].rename(columns={'symbol': 'หุ้น', 'VI Score': 'คะแนน VI', 'price': 'ราคา', 'margin_of_safety': 'MOS (%)', 'returnOnEquity': 'ROE'}).style.format({
                    'ราคา': '{:.2f}', 'MOS (%)': '{:.2f}%', 'ROE': '{:.2%}'
                }),
                hide_index=True,
                use_container_width=True
            )
        ...
        # Apply formatting
        embed_trend_col = st.checkbox("ฝัง Trend (10Y) ในคอลัมน์ของตาราง", value=True)
        if embed_trend_col:
            try:
                max_rows_embed = 100
                ...
                # 1. Fetch EPS Trend
                eps_map_embed = utils.get_eps_10_years(target_symbols, years=10)
                ...
                target_years2 = list(range(current_year2 - 10, current_year2))
                ...
                display_df_embed['Trend (10Y)'] = display_df_embed['หุ้น'].apply(_make_trend_image)
                ...
                base_cols = [c for c in cols if c not in ['Trend (10Y)', 'Div Trend']]
                # Find index of 'หุ้น'
                idx_sym = base_cols.index('หุ้น')
                
                # Reconstruct order: หุ้น, Trend, Div Trend, ...rest
                col_order = ['หุ้น', 'Trend (10Y)', 'Div Trend'] + base_cols[idx_sym+1:]
                
                cfg_embed = {
                    'หุ้น': st.column_config.TextColumn('หุ้น', width='small', pinned=True),
                    'Trend (10Y)': st.column_config.ImageColumn('Trend (10Y)', width=120, help="กำไรต่อหุ้น (EPS) ย้อนหลัง 10 ปี (ThaiFin)"),
...
elif page == "🧭 แผนลงทุนมือใหม่":
    st.title("🧭 แผนลงทุนมือใหม่")
    st.markdown("เลือกแค่ 3 อย่าง แล้วทำตามแผนแบบเป็นขั้นตอน")
    c_in1, c_in2, c_in3 = st.columns(3)
    with c_in1:
        risk_profile = st.selectbox("ระดับความเสี่ยง", ["ต่ำ", "ปานกลาง", "สูง"], index=1)
    with c_in2:
        monthly_budget = st.number_input("งบลงทุนต่อเดือน (บาท)", min_value=500, step=500, value=5000)
    with c_in3:
        horizon_years = st.slider("ระยะเวลาลงทุน (ปี)", min_value=1, max_value=20, value=5)

    risk_cfg = {
        "ต่ำ": {"equity_pct": 0.60, "min_vi": 6, "acc_mult": 1.15, "buy_mult": 1.05, "hold_mult": 1.00, "sell_mult": 0.50},
        "ปานกลาง": {"equity_pct": 0.80, "min_vi": 7, "acc_mult": 1.40, "buy_mult": 1.10, "hold_mult": 1.00, "sell_mult": 0.30},
        "สูง": {"equity_pct": 1.00, "min_vi": 7, "acc_mult": 1.60, "buy_mult": 1.20, "hold_mult": 1.00, "sell_mult": 0.00},
    }
    cfg = risk_cfg.get(risk_profile, risk_cfg["ปานกลาง"])

    set_metrics = fetch_set_index_metrics()
    set_signal = "ถือ"
    set_reason = "ยังไม่มีสัญญาณตลาดไทย (ข้อมูลไม่พร้อม)"
    set_price = None
    set_dist200 = None
    set_rsi = None
    if set_metrics:
        set_price = set_metrics.get("current")
        set_dist200 = set_metrics.get("dist200")
        set_rsi = set_metrics.get("rsi14")
        if set_dist200 is not None and set_rsi is not None:
            if set_dist200 <= -8 and set_rsi <= 35:
                set_signal = "ทยอยซื้อสะสม"
                set_reason = "SET ต่ำกว่า SMA200 มากและ RSI ต่ำ"
            elif set_dist200 <= -3 and set_rsi <= 45:
                set_signal = "ซื้อ"
                set_reason = "SET ต่ำกว่าค่าเฉลี่ยระยะยาวและเริ่มอ่อนแรง"
            elif set_dist200 >= 10 and set_rsi >= 65:
                set_signal = "ขาย"
                set_reason = "SET สูงกว่าค่าเฉลี่ยระยะยาวมากและ RSI สูง"
            else:
                set_signal = "ถือ"
                set_reason = "สัญญาณเป็นกลาง"

    mult = cfg["hold_mult"]
    if set_signal == "ทยอยซื้อสะสม":
        mult = cfg["acc_mult"]
    elif set_signal == "ซื้อ":
        mult = cfg["buy_mult"]
    elif set_signal == "ขาย":
        mult = cfg["sell_mult"]

    equity_budget = int(round(monthly_budget * cfg["equity_pct"] * mult))
    cash_budget = int(round(monthly_budget - equity_budget))

    st.markdown("---")
    c_s1, c_s2, c_s3, c_s4 = st.columns(4)
    c_s1.metric("สัญญาณ SET วันนี้", set_signal)
    if set_price is not None:
        c_s2.metric("SET ล่าสุด", f"{set_price:,.2f}")
    else:
        c_s2.metric("SET ล่าสุด", "N/A")
    if set_dist200 is not None:
        c_s3.metric("ห่าง SMA200", f"{set_dist200:+.2f}%")
    else:
        c_s3.metric("ห่าง SMA200", "N/A")
    if set_rsi is not None:
        c_s4.metric("RSI(14)", f"{set_rsi:.1f}")
    else:
        c_s4.metric("RSI(14)", "N/A")

    st.info(f"เหตุผล: {set_reason}")

    st.markdown("### ✅ สิ่งที่ควรทำเดือนนี้")
    st.markdown(f"- เงินลงทุนตามแผน: {monthly_budget:,.0f} บาท/เดือน")
    st.markdown(f"- แนะนำลงหุ้น: {equity_budget:,.0f} บาท | เงินสด/พักรอ: {cash_budget:,.0f} บาท")

    if last_fetch_time:
        now_ts = datetime.datetime.now(last_fetch_time.tzinfo) if getattr(last_fetch_time, "tzinfo", None) else datetime.datetime.now()
        cache_age_min = (now_ts - last_fetch_time).total_seconds() / 60
        if cache_age_min > (MARKET_DATA_TTL_SECONDS / 60) * 1.2:
            st.warning("ข้อมูลตลาดอาจเก่ากว่า TTL แนะนำกดปุ่ม “อัปเดตข้อมูลราคาและงบการเงิน” ก่อนทำรายการ")

    st.markdown("### 🧩 หุ้นแนะนำสำหรับมือใหม่ (กระจายความเสี่ยง)")
    picks = []
    if df is not None and not df.empty:
        cand = df.copy()
        cand = cand[(cand["price"] > 0) & (cand["VI Score"] >= cfg["min_vi"])]
        cand["mos_pick"] = cand["margin_of_safety_raw"].fillna(-9999)
        cand = cand.sort_values(by=["mos_pick", "VI Score"], ascending=False)
        preferred = cand[cand["mos_pick"] > 0]
        pool = preferred if not preferred.empty else cand

        seen_sectors = set()
        for _, row in pool.iterrows():
            sector = row.get("sector") or "Unknown"
            if sector in seen_sectors and len(seen_sectors) < 5:
                continue
            picks.append(row)
    ...
    if not picks or equity_budget <= 0:
        st.warning("ยังไม่สามารถสร้างรายการหุ้นแนะนำได้ (อาจเพราะข้อมูลไม่ครบ หรือสัญญาณแนะนำให้พักเงิน)")
    else:
        per_stock = int(round(equity_budget / len(picks)))
        pick_rows = []
        for row in picks:
            sym = row.get("symbol")
            pick_rows.append({
                ...
                "งบแนะนำ (บาท)": per_stock,
            })
        st.dataframe(pd.DataFrame(pick_rows), hide_index=True, use_container_width=True)

    st.markdown("### 📌 กติกาที่แผนนี้ใช้")
    st.markdown("- ซื้อแบบ DCA ทุกเดือนตามงบที่กำหนด")
    st.markdown("- ถ้า SET เข้าโซน “ทยอยซื้อสะสม” จะเพิ่มงบซื้อ (ตามระดับความเสี่ยง)")
    st.markdown("- ถ้า SET เข้าโซน “ขาย” จะลด/หยุดซื้อเพื่อพักเงินสด (ตามระดับความเสี่ยง)")
    st.markdown("- รายชื่อหุ้นเลือกจากคะแนนคุณภาพ (VI Score) และความถูกแพง (MOS) พร้อมพยายามกระจายกลุ่มธุรกิจ")

    st.markdown("### 🔮 เป้าหมายมูลค่าในอนาคต (ประมาณการ)")
    rate = st.slider("สมมติผลตอบแทนเฉลี่ยต่อปี (%)", min_value=0.0, max_value=15.0, value=8.0, step=0.5)
    r = rate / 100 / 12
    n = horizon_years * 12
    fv = monthly_budget * (((1 + r) ** n - 1) / r) if r > 0 else monthly_budget * n
    st.metric("มูลค่าเป้าหมายโดยประมาณ", f"{fv:,.0f} บาท")
    st.caption("เป็นการประมาณทางคณิตศาสตร์เพื่อวางแผน ไม่ใช่การรับประกันผลตอบแทน")
    ...
    if len(selected_tickers) > 0:
        if len(selected_tickers) > 5:
            ...
            with st.spinner("กำลังดึงข้อมูลเปรียบเทียบ..."):
                ...
                comp_df_disp['roe_pct'] = comp_df_disp['returnOnEquity'] * 100
                comp_df_disp['npm_pct'] = comp_df_disp['profitMargins'] * 100
                
                # Select interesting columns
                cols_to_show = ['price', 'fair_value', 'margin_of_safety', 'dividendRate', 'roe_pct', 'npm_pct', 'debtToEquityRatio', 'pe_ratio']
                comp_table = comp_df_disp[cols_to_show].T
    ...
    with st.expander("➕ เพิ่มรายการซื้อ/ขาย (Add Transaction)", expanded=False):
        ...
        if st.button("บันทึกรายการ"):
            ok, err = utils.save_transaction(t_symbol, t_date, t_price, t_qty, t_open_action)
            if ok:
                st.success(f"บันทึก {t_open_action} {t_symbol} เรียบร้อย!")
                st.rerun()
            else:
                st.error(err or "บันทึกรายการไม่สำเร็จ")
    ...
    with st.expander("สถานะอัปเดตรายชื่อ SET100 อัตโนมัติ", expanded=True):
        ok = meta.get("ok")
        last_checked_at = meta.get("last_checked_at") or "-"
        last_updated_at = meta.get("last_updated_at") or "-"
        doc_name = meta.get("doc_name") or "-"
        c1, c2, c3 = st.columns(3)
        c1.metric("ตรวจสอบล่าสุด", str(last_checked_at))
        c2.metric("อัปเดตล่าสุด", str(last_updated_at))
        c3.metric("เอกสารอ้างอิง", str(doc_name))
        if ok is False:
            st.error(meta.get("error") or "อัปเดตอัตโนมัติไม่สำเร็จ")
        else:
            added = meta.get("added") or []
            removed = meta.get("removed") or []
            if added or removed:
                st.warning(f"มีการเปลี่ยนรายชื่อ: เข้า {len(added)} ออก {len(removed)}")
                if added:
                    st.write("เข้า:", ", ".join(added))
                if removed:
                    st.write("ออก:", ", ".join(removed))
            else:
                st.success("ยังไม่พบการเปลี่ยนแปลงรายชื่อ")

        if st.button("ตรวจสอบ SET100 ล่าสุดทันที"):
            with st.spinner("กำลังดึงรายชื่อ SET100 ล่าสุดจากเว็บ SET..."):
                res = utils.update_set100_tickers_if_needed(force=True)
            if res.get("ok") and res.get("changed"):
                st.success(f"อัปเดตรายชื่อสำเร็จ: เข้า {len(res.get('added') or [])} ออก {len(res.get('removed') or [])}")
                st.cache_data.clear()
                st.rerun()
            elif res.get("ok"):
                st.info("ไม่มีการเปลี่ยนแปลงรายชื่อ")
                st.cache_data.clear()
                st.rerun()
            else:
                st.error(res.get("error") or "อัปเดตไม่สำเร็จ")
